...
//! alongside it.
//!
//! Config comes from a Profile, not the environment. A run that ends early is
//! interrupted rather than cancelled, so it still reports as `degraded` with
//! what was measured.
...
pub const Options = struct {
    ...
    target: []const u8,
    /// Where to sample container CPU/memory, if anywhere.
...
const ProgressCtx = struct {
    ...
    arena: Allocator,
    /// Flushed after every window: the .ndjson is the only artifact a wedged
    /// ramp leaves (run #21).
    nd: *Io.File.Writer,
...
/// Heartbeat interval: shows in the journal where a wedged ramp stopped.
...
fn onProgress(
    ...
    // Reuse zrk's math so the NDJSON matches the zrk CLI byte-for-byte.
    ...
    // Heartbeat. Per-interval rate, not cumulative, so a collapse shows.
...
/// Raise `flag` after `ns`: a backstop turning a zrk hang into a truncated
/// result.
...
/// One proxy's ramp.
///
/// Allocates from a ramp-scoped arena, never the process-lifetime caller arena:
/// the zio stack pool is ~2.7 GiB at c10k and would leak per proxy until OOM.
pub fn run(gpa: Allocator, caller_arena: Allocator, opts: Options) !Outcome {
    ...
    // Fail legibly before measuring rather than with EMFILE mid-ramp.
    ...
    var cfg: cli.Config = .{
        ...
        // Shed-before-send only; see Profile.deadline_ms for why never abort.
        .deadline_ns = p.deadlineNs(),
        .deadline_abort = false,
        .record_timeouts = false,
        // No verification: the target is an IP literal with a per-run
        // self-signed cert inside the VPC. Tied to the profile so a plaintext
        // profile never quietly runs an unverified https target.
        .insecure = p.tls,
        // No zrk whole-run JSON: it embeds the target URL (a private address)
        // in a published file (zrk/src/report.zig writeUrl).
        .format = .json,
        .output_path = null,
        .hdr_path = hgrm_path,
    ...
    // Profile and URL come from different places; a mismatch would fail every
    // connection or publish plaintext numbers as TLS.
    ...
    if (opts.cadvisor_addr != null) {
        ...
        for (samples.items) |s| {
            // cadvisor_ms is the counter's own timestamp; see cadvisor.Sample.
    ...
    if (opts.cadvisor_addr != null) {
        // Distinguishes "never reached cAdvisor" from "never found the
        // container".
...
/// Minimum coverage worth reporting; below it no knee can be placed, so the
/// result is `failed` rather than `degraded`.
...
//
// `ramp.run` runs as a child (`bench ramp`) so a wedged generator can be killed
// on a deadline: once `runner.run` blocks, zio owns the thread.
...
test "an Outcome survives the child/parent round-trip" {
    ...
    // A silently zeroed field would read as a proxy that served nothing.
    const want: Outcome = .{
        .elapsed_s = 300.408,
        .configured_s = 300.0,
        .launched = 10000,
