...
echo ">>> fleet: loadgen=$LOADGEN_PUB loadgen2=$LOADGEN2_PUB proxy=$PROXY_PUB backend=$BACKEND_PUB"
...
# --- ship the repo -------------------------------------------------------------
for host in "$LOADGEN_PUB" "$LOADGEN2_PUB" "$PROXY_PUB" "$BACKEND_PUB"; do
    echo ">>> rsync -> $host"
    rsync -az --delete \
...
# loadgen2 runs k6 only — but bring up its exporters so its host CPU is scraped
# (the whole point of a 2nd loadgen is to NOT be CPU-bound; we must see that),
# and create results/ for k6's handleSummary write.
ssh -o BatchMode=yes "$SSH_USER@$LOADGEN2_PUB" "cd $REMOTE_DIR && $COMPOSE --profile monitoring up -d --wait cadvisor node_exporter && mkdir -p results"
...
MODE=cloud \
    ...
    LOADGEN2_SSH="$SSH_USER@$LOADGEN2_PUB" \
