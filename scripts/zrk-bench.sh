...
# report/report.py. One 16-core loadgen saturates any proxy at ~25% CPU, so
# there is no second loadgen.
#
...
echo ">>> runid=$RUNID proxies=[$PROXIES] ramp=$START_RATE->${MAX_RATE}rps/${RAMP_SECONDS}s conns=$CONNECTIONS cpus=$PROXY_CPUS"

# --- ship repo to proxy+backend; ensure backend, exporters, zrk built -----------
...
# config (prometheus job + live dashboard) so Grafana shows the ramp live.
# file_sd auto-reloads the target; the prometheus.yml `zrk` job is a one-time
# add (restart prometheus once after first deploy).
mkdir -p monitoring/targets/cloud
cat > monitoring/targets/cloud/zrk.yml <<EOF
# regenerated by zrk-bench.sh
